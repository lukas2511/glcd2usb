...
const PROGMEM char usbHidReportDescriptor[USB_CFG_HID_REPORT_DESCRIPTOR_LENGTH] = {
    ...
    0x85, GLCD2USB_RID_WRITE_192,  //   REPORT_ID
    0x95, 192+3,                   //   REPORT_COUNT (131)
    0x09, 0x00,                    //   USAGE (Undefined)
    0xb2, 0x02, 0x01,              //   FEATURE (Data,Var,Abs,Buf)
...
uchar usbFunctionWrite(uchar *data, uchar len)
{
  ...
  case GLCD2USB_RID_WRITE:
    ...
    if(cmd_state.offset == 0xffff) {
      ...
      /* set draw cursor */
      glcdSetAddress(cmd_state.offset%192, cmd_state.offset/192);
    }
...
void whirl_init(void) {
  ...
  rprintf("V: %d.%2d", VERSION_MAJOR, VERSION_MINOR);
...
int	main(void) {
  ...
  TIMSK0 = 5;         /* timer 0 prescaler: 1024 */
