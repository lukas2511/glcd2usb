...
int main(int argc, char **argv)
{
  ...
  do {
	...
	if((bright&0x7fff) > 255) 
      ...
      /* the 192x64 display is filled in 8 transfers with 192 bytes each */
      for(j=0;j<8;j++) {
	int n;
	
	buffer.bytes[0] = GLCD2USB_RID_WRITE_192;
	buffer.bytes[1] = (j*192)%256;
	buffer.bytes[2] = (j*192)/256;
	buffer.bytes[3] = 192;          // real length
...
/*	buffer.bytes[1] = (j*128)%256;
	...
	if((err = usbSetReport(dev, USB_HID_REPORT_TYPE_FEATURE, buffer.bytes, 196)) != 0) {
